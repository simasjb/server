...
void log_t::file::open_file(std::string path)
{
  ...
  if (!fd.open())
    ib::fatal() << "open(" << fd.get_path() << ") failed";
}
...
void log_t::file::flush_data_only()
{
  ...
  if (!fd.flush_data_only())
    ib::fatal() << "flush_data_only(" << fd.get_path() << ") failed";
...
{
  if (fd.is_opened() && !fd.close())
    ib::fatal() << "close(" << fd.get_path() << ") failed";
}
